% Sauer p.318 Program 6.5 Hodgkin-Huxley equations
...
function y = HodgkinHuxley(inter, ic, n)
    global pa pb pulse % user inputs vals for these 3 input values
    inp = input ('pulse start, end, muamps in [ ], e.g. [50 51 7]: ');
    pa = inp(1);pb = inp(2);pulse = inp(3);
    a = inter(1);b=inter(2);h = (b-a)/n; %time start,end,step size
    y(1,:) = ic; %first row of y as initial conditions
    t(1) = a;
    for i =1:n
        t(i+1) = t(i) + h;
...
function y=rk4step(t,w,h)
    %one step of Runga-Kutta O(4) method
    s1 = ydot(t,w);
    s2 = ydot(t + h/2, w+h*s1/2);
    s3 = ydot(t + h/2, w+h*s2/2);
    s4 = ydot( t + h, w + h*s3);
    y = w + (s1 + 2*s2 + 2*s2 + s4)/6;
    
function z=ydot(t,w)
    % define the Hodgkin-Huxley DEs where z returns derivative?
    global pa pb pulse % why globals?
    c=1;g1=120;g2=0.3;T=(pa+pb)/2;len=pb-pa;
    e0=-65;e1=50;e2=-77;e3=-54.4;
    in=pulse*(1 - sign(abs(t-T)-len/2))/2;
    % square pulse input on interval [pa,pb] of pulse mu_amps
        
    v = w(1); m=w(2); n=w(3); h=w(4);
    z = zeros(1,4);
        
    z(1) = (in - g1*m*m*m*h*(v-e1) - g2*n*n*(v-e2)-g3*(v-e3))/c;
    v = v-e0;
...
HodgkinHuxley([0,100],[-65,0,0.3,0.6],2000)
